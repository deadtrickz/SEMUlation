...
$RomsDirectory = Read-Host "Enter the path to the ROMs folder. Not the specific game system folder"
...
foreach ($subDir in $subDirectories) {
    ...
    $mediaSubDir = Join-Path -Path $romSubDir -ChildPath "videos"

    if ((Test-Path $romSubDir) -and (Test-Path $mediaSubDir)) {
        ...
        $videoNames = $videoFiles.BaseName | ForEach-Object {
            $_ -replace '-video$', ''
        } | Select-Object -Unique

        $missingVideos = Compare-Object -ReferenceObject $uniqueRomNames -DifferenceObject $videoNames |
        ...
        if ($missingVideos.Count -gt 0) {
            ...
            if ($OutputToFile -eq 'Y') {
                ...
                $missingVideos | ForEach-Object { Write-Host $_ }
            }
        }
    }
}
