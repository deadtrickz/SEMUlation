...
function Index-RemoteFiles {
    param ($files, $type)
    $index = @{}
    foreach ($file in $files) {
        if ($file.BaseName -match "-$type$") {
            $rawName = $file.BaseName -replace "-$type$", ""
            $clean = Get-CleanName $rawName
            if (-not $index.ContainsKey($clean)) {
                $index[$clean] = $file
            }
        }
    }
...
function Update-System {
    ...
    foreach ($type in $imageTypes) {
        ...
        $remoteImageIndex[$type] = Index-RemoteFiles $files $type
    }

    $remoteVideoIndex = if (Test-Path -LiteralPath $remoteVideos) {
        Index-RemoteFiles (Get-ChildItem -LiteralPath $remoteVideos -File) $videoType
    } else {
        @{}
    }
    ...
    foreach ($rom in $romDict.Keys) {
        ...
        foreach ($type in $imageTypes) {
            $match = $remoteImageIndex[$type][$clean]
            if ($match) {
        ...
        $videoMatch = $remoteVideoIndex[$clean]
        if ($videoMatch) {
            $dest = Join-Path $localVideos "$rom-$videoType$($videoMatch.Extension)"
            if (!(Test-Path -LiteralPath $dest)) {
                Copy-Item -LiteralPath $videoMatch.FullName -Destination $dest
            }
        } else {
            $missing += "-$videoType"
        }
