# This script compares the files in two directories and shows which files are unique to each directory after normalizing file names.

function Normalize-FileName($fileName) {
    return ($fileName -replace '\[.*?\]|\(.*?\)|_|-01|-02|.ps3| -', '' -replace '\s+', ' ').Trim()
}
...
# Retrieve and normalize file base names from both directories
$files1 = Get-ChildItem -Path $Directory1 -File | ForEach-Object { Normalize-FileName $_.BaseName }
$files2 = Get-ChildItem -Path $Directory2 -File | ForEach-Object { Normalize-FileName $_.BaseName }
