    ...
    if ($ImageFiles) {
        ...
        foreach ($ImageFile in $ImageFiles) {
            if ($RomFilenames -contains $ImageFile.BaseName) {
                ...
                $destinationFilePath = Join-Path -Path $DestinationImagesDirectory -ChildPath ($ImageFile.BaseName + $suffix + $ImageFile.Extension)
                if (-not (FileExistsWithBaseName -destinationPath $DestinationImagesDirectory -baseName $ImageFile.BaseName)) {
...
if ($VideoFiles) {
    foreach ($RomBaseName in $RomFilenames) {
        ...
        if ($matchingVideo) {
            ...
            $destinationFilePath = Join-Path -Path $DestinationVideosDirectory -ChildPath ($RomBaseName + $VideoSuffix + $matchingVideo.Extension)
            if (-not (FileExistsWithBaseName -destinationPath $DestinationVideosDirectory -baseName $RomBaseName)) {
