# This script moves video files from a remote directory to a local directory,
...
# Move and rename video files to match ROM names
    ...
    foreach ($RomItem in $RomItems) {
        ...
        if ($normalizedRomName -eq $normalizedVideoName) {
            ...
            if (-not (Test-Path -Path $destinationFilePath)) {
                Move-Item -Path $VideoFile.FullName -Destination $destinationFilePath
                $filesMoved++
                Write-Host "Moved and renamed $filesMoved/$totalFiles '$($VideoFile.Name)' to match '$($RomItem.Name)'"
            } else {
...
Write-Host "Operation completed. Total files moved: $filesMoved"
Pause
