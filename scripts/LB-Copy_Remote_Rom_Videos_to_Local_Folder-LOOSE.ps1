# This script copies Launchbox video files from a remote directory to a local directory,
...
$VideoDirectory = Read-Host "Enter the path to the REMOTE Launchbox video directory"
...
foreach ($VideoFile in $VideoFiles) {
    ...
    foreach ($RomItem in $RomItems) {
        ...
        if ($normalizedRomName -eq $normalizedVideoName) {
            $destinationFilePath = Join-Path -Path $DestinationDirectory -ChildPath ($RomItem.Name)
